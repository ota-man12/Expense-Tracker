...
namespace MyApp.Namespace
{
    public class DashboardController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            //Line Chart
            List<LineChartData> IncomeSummary = SelectedTransactions.Where(i => i.Category.Type == "Income").GroupBy(j => j.Date).Select(k => new LineChartData()
            {
                day = k.First().Date.ToString("dd-MMM"),
                income = k.Sum(l => l.Amount)
            }).ToList();

            List<LineChartData> ExpenseSummary = SelectedTransactions.Where(i => i.Category.Type == "Expense").GroupBy(j => j.Date).Select(k => new LineChartData()
            {
                day = k.First().Date.ToString("dd-MMM"),
